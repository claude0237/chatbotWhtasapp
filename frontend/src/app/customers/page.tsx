...
function Avatar({ name, phone, size = 'md' }: { name?: string | null; phone: string; size?: 'sm' | 'md' | 'lg' }) {
  ...
  return (
    <div className={`${sz} rounded-full bg-green-100 flex items-center justify-center flex-shrink-0 text-green-700 font-bold`}>
      {letter}
    </div>
  );
}
    ...
    <AppLayout>
      <div className="flex flex-col h-full" style={{ height: 'calc(100vh - 112px)' }}>
        ...
        <div className="flex items-center justify-between mb-4 flex-shrink-0">
          <div>
            <h1 className="text-xl font-bold text-gray-900">👤 Carnet de contacts</h1>
            <p className="text-xs text-gray-400 mt-0.5">{totalCustomers} contact(s) WhatsApp</p>
          </div>
          <input value={search} onChange={e => setSearch(e.target.value)}
            placeholder="🔍 Nom, téléphone, email…"
            className="border border-gray-300 rounded-lg px-3 py-1.5 text-sm w-64 focus:ring-2 focus:ring-green-500"
          />
        </div>

        {error && <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-2 rounded-lg mb-3 flex justify-between text-sm flex-shrink-0"><span>{error}</span><button onClick={() => setError('')}>✕</button></div>}
        {success && <div className="bg-green-50 border border-green-200 text-green-700 px-4 py-2 rounded-lg mb-3 flex justify-between text-sm flex-shrink-0"><span>✅ {success}</span><button onClick={() => setSuccess('')}>✕</button></div>}

        <div className="flex gap-4 flex-1 min-h-0">

          {/* ── Liste contacts ── */}
          <div className="w-64 flex-shrink-0 bg-white rounded-xl shadow flex flex-col overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto divide-y divide-gray-100">
              {loading ? (
                ...
                <div key={c.id} onClick={() => openCustomer(c)}
                  className={`px-3 py-3 cursor-pointer hover:bg-gray-50 transition-colors ${selected?.id === c.id ? 'bg-green-50 border-l-4 border-green-500' : 'border-l-4 border-transparent'}`}>
                  <div className="flex items-center gap-2.5">
                    <Avatar name={c.name} phone={c.phone_number} size="sm" />
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-gray-900 truncate">{c.name || <span className="text-gray-400 italic">Inconnu</span>}</p>
          ...
          <div className="flex-1 min-w-0 flex flex-col gap-4 overflow-y-auto">
            {selected ? (
              <>
                ...
                <div className="bg-white rounded-xl shadow p-5">
                  <div className="flex items-start justify-between gap-4 mb-5">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h2 className="text-lg font-bold text-gray-900">{selected.name || <span className="text-gray-400 italic">Nom inconnu</span>}</h2>
                        <p className="text-sm text-gray-500">📱 {selected.phone_number}</p>
                        {selected.extra_data?.email && <p className="text-sm text-gray-500">✉️ {selected.extra_data.email}</p>}
                        {selected.extra_data?.company_name && <p className="text-sm text-gray-500">🏢 {selected.extra_data.company_name}</p>}
                      </div>
                    </div>
                  ...
                  {!editing && selected.extra_data?.tags && (selected.extra_data.tags as string[]).length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mb-4">
                      {(selected.extra_data.tags as string[]).map((t, i) => (
                        <span key={i} className="text-xs bg-indigo-50 text-indigo-700 border border-indigo-100 px-2 py-0.5 rounded-full font-medium">{t}</span>
                      ))}
                    </div>
                  )}
                  ...
                  {!editing && selected.extra_data?.notes && (
                    <div className="mt-3 bg-amber-50 border border-amber-100 rounded-lg p-3">
                      <p className="text-xs text-amber-600 font-medium mb-1">📝 Notes internes</p>
                      <p className="text-sm text-gray-700 whitespace-pre-wrap">{selected.extra_data.notes}</p>
                    </div>
                  )}
                  ...
                  {editing && (
                    <form onSubmit={handleSave} className="space-y-4">
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <div>
                          <label className="block text-xs font-medium text-gray-600 mb-1">Source d'acquisition</label>
                ...
                <div className="bg-white rounded-xl shadow p-5">
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="font-semibold text-gray-900 text-sm">💬 Historique des conversations</h3>
                    <Link href={`/conversations`}
                      className="text-xs text-green-600 hover:underline">Voir toutes →</Link>
                  </div>
                  {convLoading ? (
                    ...
                    <div className="space-y-2">
                      {conversations.map(c => (
                        <div key={c.id} className="flex items-center justify-between bg-gray-50 rounded-lg px-3 py-2.5 hover:bg-gray-100 transition-colors">
              ...
              <div className="flex-1 bg-white rounded-xl shadow flex flex-col items-center justify-center text-gray-400 min-h-64">
